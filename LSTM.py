...
def inputData(fPath):
	...
	for fidx, file_name in enumerate(os.listdir(fPath)[:NUMOFFILE]):
		...
		for sheet_idx in range(1, SHEET_SIZE+1):
			...
			#시트 내에서 한 시점씩 읽기, 한 샘플(=윈도우구간*feature)씩 저장
			for i in range(startLine, endLine):
				sample_x = []
				sample_y = []
				#윈도우구간 내에서 한 시점씩 읽기 ( i-win_size ~ i )
...
def template(files, sheet_size, nTrain, dType="int"):
	...
	#lstm 입력구조 변환 (samples, timesteps, features)
	if dType == "float":
...
if __name__ == "__main__":
	...
	with open(OUTFILEPATH+"result.csv","w") as f:
		for idx, line in enumerate(predict):
			...
			if EXCEPTION:	# 레이블 0을 제외했을 경우 데이터 입력
				p2 = argmax(p1[1:])+1
			else:			#모든 레이블을 고려했을 경우 데이터 입력
				p2 = argmax(p1)
